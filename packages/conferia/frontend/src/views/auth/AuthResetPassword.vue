        ...
        <div class="w-full max-w-md space-y-8">
          ...
          <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 p-8">
            <form @submit.prevent="sendResetEmail" class="space-y-6">
              ...
              <div v-if="resetEmailError" class="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-700 rounded-lg p-3">
                <p class="text-red-700 dark:text-red-300 text-sm">{{ resetEmailError }}</p>
              </div>

              <div v-if="resetEmailSuccess" class="bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-700 rounded-lg p-3">
                <p class="text-green-700 dark:text-green-300 text-sm">{{ resetEmailSuccess }}</p>
              </div>
            </form>
          </div>
        </div>
