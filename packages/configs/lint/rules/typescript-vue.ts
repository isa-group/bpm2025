...
const common = (packageName: string) => defineConfig([
  {
    ...
    name: '(@bpm2025-website/configs/lint/typescript-vue - typescript-eslint) Extended config from plugin (strict type checking)'
  },
  {
    ...flatArrayOfObjects(tseslint.configs.stylisticTypeChecked),
    name: '(@bpm2025-website/configs/lint/typescript-vue - typescript-eslint) Extended config from plugin (stylistic type checking)'
  },
  {
    ...tseslint.configs.eslintRecommended,
    name: '(@bpm2025-website/configs/lint/typescript-vue - typescript-eslint) Extended config from plugin (ESLint rules with type checking)'
  },
  {
    ...regexp.configs[recommendedKey],
    name: '(@bpm2025-website/configs/lint/typescript-vue - regexp) Extended config from plugin'
  },
  {
    ...promise.configs[recommendedKey],
    name: '(@bpm2025-website/configs/lint/typescript-vue - promise) Extended config from plugin'
  },
  {
    name: '(@bpm2025-website/configs/lint/typescript-vue - promise) Custom config',
    rules: {
  ...
  {
    name: '(@bpm2025-website/configs/lint/typescript-vue - import) Custom config',
    plugins: {
  ...
  {
    name: '(@bpm2025-website/configs/lint/typescript-vue - JSDoc) Custom config',
    plugins: {
      jsdoc
    },
    rules: {
   ...
   */
  {
    name: '(@bpm2025-website/configs/lint/typescript-vue - TypeScript & Vue) Disable no-unsafe-* rules',
    rules: {
  ...
  {
    name: '(@bpm2025-website/configs/lint/typescript-vue - TypeScript & Vue) Custom config',
    rules: {
...
const vue_config = defineConfig([
  ...vue.configs[recommendedKey].map((config) => {
    ...
    config.name = `(@bpm2025-website/configs/lint/typescript-vue - ${config.name}) - Extended config from plugin`;

    return config;
  }),
  {
    ...flatArrayOfObjects(vueScopedCSS.configs[recommendedKey]),
    name: '(@bpm2025-website/configs/lint/typescript-vue - Vue Scoped CSS) Extended config from plugin',
    files: vueFiles
  },
  {
    ...css.configs[recommendedKey],
    name: '(@bpm2025-website/configs/lint/typescript-vue - Vue CSS) Extended config from plugin',
    files: vueFiles
  },
  {
...
export function getTSVueConfig(packageName: string, enableVue = true, tsconfigRootDir = import.meta.dirname) {
  ...
  const base_vue_parser = {
    name: '(@bpm2025-website/configs/lint/typescript-vue - Vue) Extra parser configuration for typed linting',
    files: vueFiles,
    languageOptions: {
      ...langOptions,
      parser: vueParser,
  ...
  const base_ts_parser = {
    name: enableVue
      ? '(@bpm2025-website/configs/lint/typescript-vue - TypeScript & Vue) Parser extra config'
      : '(@bpm2025-website/configs/lint/typescript-vue - TypeScript) Parser extra config',
